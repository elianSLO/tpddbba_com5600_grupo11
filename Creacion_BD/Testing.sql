...
-- CASO 1.1.3: Email inv�lido (debe fallar por validaci�n)
...
-- CASO 1.1.6: Tel�fono con letras (debe fallar)
-- Nota: Se utiliza la misma validaci�n para tel�fono auxiliar y tel�fono de cobertura
...
-- Inserci�n de socio para pruebas
...
-- CASO 1.2.1: Modificaci�n v�lida
...
-- CASO 1.2.2: C�digo de socio no existente (debe fallar)
...
-- CASO 1.2.4: Email inv�lido (debe fallar)
...
-- CASO 1.2.5: Tel�fono con letras (debe fallar)
...
-- CASO 2.1.3: Email inv�lido (debe fallar por validaci�n)
...
-- CASO 2.1.6: Tel�fono con letras (debe fallar)
...
-- CASO 2.2.1: Modificaci�n v�lida
...
-- CASO 2.2.2: C�digo de socio no existente (debe fallar)
...
-- CASO 2.2.4: Email inv�lido (debe fallar)
...
-- CASO 2.2.5: Tel�fono con letras (debe fallar)
...
-- Aseg�rate de que la tabla psn.Profesor existe antes de ejecutar las pruebas

-- Limpiar la tabla para pruebas (solo si es seguro)
DELETE FROM psn.Profesor
DBCC CHECKIDENT ('psn.Profesor', RESEED, 0);

-- 3.1.1. Inserci�n exitosa
...
-- 3.1.3. DNI inv�lido (menos de 8 caracteres)
EXEC stp.insertarProfesor 
    @dni = '12345',
    @nombre = 'Ana',
    @apellido = 'L�pez',
...
-- 3.1.5. Tel�fono con caracteres no num�ricos
EXEC stp.insertarProfesor 
    @dni = '23456789',
    @nombre = 'Mario',
    @apellido = 'Torres',
...
-- CASO 3.2.1: Modificaci�n v�lida
...
-- CASO 3.2.3: DNI inv�lido (menos de 8 d�gitos, debe fallar)

EXEC stp.modificarProfesor
    @cod_prof = 1,
    @dni = '1234567', -- 7 d�gitos
    @nombre = 'Laura',
    @apellido = 'Martinez',
    @email = 'laura.martinez@correo.com',
    @tel = '1134567891';

-- CASO 3.2.4: Nombre con caracteres inv�lidos (debe fallar)
...
-- CASO 3.2.5: Apellido con n�meros (debe fallar)
...
-- CASO 3.2.6: Email inv�lido (sin arroba, debe fallar)
...
EXEC stp.modificarProfesor
    ...
    @nombre = 'Diego',
    @apellido = 'Sosa',
    @email = 'diego.sosaemail.com', -- inv�lido
    @tel = '1134567894';

-- CASO 3.2.7: Tel�fono con letras (debe fallar)
...
EXEC stp.modificarProfesor
    ...
    @tel = '11345ABCD'; -- inv�lido

-- CASO 3.2.8: Tel�fono muy corto (debe fallar)
...
EXEC stp.modificarProfesor
    ...
    @tel = '123456'; -- menos de 10 d�gitos
...
-- 4.1 PRUEBA DE INSERCI�N DE PAGO

EXEC stp.insertarPago
	@monto = 1500.00,
	@fecha_pago = '2025-06-10',
	@estado = 'Pagado',
	@cod_socio = 1,  -- Asegurarse que este socio exista, sino dar� error
	@cod_invitado = NULL;
GO

select * from psn.Pago
-- 4.2 PRUEBA DE MODIFICACI�N DE PAGO
...
-- 5.1 PRUEBA DE INSERCI�N DE RESPONSABLE
...
-- 5.2 PRUEBA DE MODIFICACI�N DE RESPONSABLE
...
EXEC stp.modificarReembolso
    @codReembolso = 1,
    @monto = 2000.00,
    @medio_Pago = 'Tarjeta de cr�dito',
    @fecha = '2025-06-11',
    @motivo = 'Estudios cl�nicos';
