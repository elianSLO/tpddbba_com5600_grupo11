...
BEGIN
	...
	WITH FacturasVencidas AS (
		...
		INNER JOIN Persona.Socio s ON f.cod_socio = s.cod_socio
...
BEGIN
    ...
    WITH IngresosMensuales AS (
        ...
        FROM Finanzas.Factura F
        INNER JOIN Finanzas.Item_Factura I ON F.cod_Factura = I.cod_Factura
        INNER JOIN Actividad.Clase C ON CAST(I.descripcion AS INT) = C.cod_clase
        INNER JOIN Club.Actividad A ON C.cod_actividad = A.cod_actividad
        WHERE 
            F.estado = 'Pagada'
...
BEGIN
    ...
    WITH DatosSocioClase AS (
        ...
        FROM Actividad.Inscripto I
        INNER JOIN Actividad.Clase C ON I.cod_clase = C.cod_clase
        INNER JOIN Club.Actividad A ON C.cod_actividad = A.cod_actividad
        INNER JOIN Club.Suscripcion S ON I.cod_socio = S.cod_socio AND S.cod_categoria = C.categoria
        INNER JOIN Club.Categoria Cat ON S.cod_categoria = Cat.cod_categoria
    ),
...
BEGIN
    ...
    WITH SociosInscriptos AS (
        SELECT 
            ...
            C.cod_clase
        FROM Actividad.Inscripto I
        INNER JOIN Persona.Socio S ON I.cod_socio = S.cod_socio
        INNER JOIN Actividad.Clase C ON I.cod_clase = C.cod_clase
        INNER JOIN Club.Actividad A ON C.cod_actividad = A.cod_actividad
    ...
    SociosConCategoria AS (
        ...
        LEFT JOIN Club.Categoria CAT ON SC.cod_categoria = CAT.cod_categoria
    ),
...
-- CATEGORÍAS
INSERT INTO Club.Categoria (descripcion, edad_max, valor_mensual, vig_valor_mens, valor_anual, vig_valor_anual)
VALUES
